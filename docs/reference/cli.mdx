...
After `cargo install --path crates/cli`, invoke `raven` directly.
...
```bash
RUST_LOG=raven=debug,raven_plugin_=debug,raven_source_rpc=debug \
  raven run --rpc-url http://localhost:8545
```

The default filter enables info logs for the CLI, statically linked
`raven-plugin-*` crates, and RPC source. Set `RUST_LOG` to

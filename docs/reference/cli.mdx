...
| `--reorg-monitor`                    | —              | Disabled     | Report applied and reverted blocks through tracing             |
...
The `reorg-monitor` plugin is statically linked but disabled until
`--reorg-monitor` is supplied:

```bash
raven run --rpc-url https://your-evm-rpc.example --reorg-monitor
```
...
Built-in plugins
  ...
  ○ reorg-monitor  disabled; enable with `raven run --reorg-monitor`
...
2026-08-31T08:10:33.012144Z  INFO [ erc20-transfer ] raven_plugin_erc20_transfer: large ERC-20 transfer ...
...
The tag is the visual boundary for consecutive lines from one plugin; Raven
keeps individual tracing events on one terminal line. Colors are deterministic
for the same plugin registration order and are distinct for the first 360
registered plugins. Source, runtime, checkpoint, and CLI orchestration logs do
not receive a plugin tag. With `NO_COLOR` or redirected output, the same
`[ plugin-name ]` text remains without ANSI color.

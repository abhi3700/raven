...
`reorg-monitor` is a bundled, statically linked Plugin that writes every
...
Installation saves an empty `reorg-monitor` configuration object because the
plugin has no plugin-specific arguments. Remove it with
`raven plugins remove reorg-monitor`.

It has no RPC client and does not independently detect forks. The RPC source

...
The RPC source fetches generic EVM logs and includes them in the normalized
`BlockEvent`. The plugin derives ERC-20 meaning from that payload; it never
polls the node itself.

```mermaid
flowchart LR
    rpc[EVM JSON-RPC] --> source[RPC source backed by Alloy]
...
```bash
RUST_LOG=raven=info,raven_plugin_erc20_transfer=debug,raven_source_rpc=info \
  raven run --rpc-url http://localhost:8545
```

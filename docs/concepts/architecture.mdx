...
flowchart TB
    ...
    subgraph planned["Planned: embedded Reth deployment"]
        ravenReth["Raven-enabled Reth binary"] --> exex["Raven ExEx task"]
...
Reth process. Raven therefore plans a separate, version-pinned Raven-enabled
Reth binary rather than `raven run --source reth`. That binary will translate
commit, revert, and reorg notifications into the same validated event model and
can use the node provider for additional state reads.
...
| Existing Reth node without Raven compiled in | Use its RPC endpoint, or restart with the planned Raven-enabled binary |
| Zero-RPC, in-process Reth access | Planned Raven ExEx integration |
| Separate Raven process receiving ExEx events | Possible future remote-ExEx adapter, not direct DB access |

See Reth's [ExEx overview](https://reth.rs/exex/overview/) and
[remote ExEx guide](https://reth.rs/exex/remote/) for the underlying execution

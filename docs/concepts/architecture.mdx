...
```mermaid
flowchart TD
    ravenCli["raven CLI"] --> rpcSource["RPC source (Alloy-backed)"]
...
flowchart TB
    subgraph current["Current: standalone RPC deployment"]
        ...
        rpc --> rpcAdapter["RPC source (Alloy-backed)"]
...
```text
standalone: raven run --rpc-url ... -> RPC source (Alloy-backed) -> Raven runtime
...
```mermaid
flowchart LR
    chain["EVM JSON-RPC"] --> source["RPC source (Alloy-backed)"]

...
The RPC source emits `BlockReverted` for retained blocks displaced by a shallow
fork, from old tip down to the common ancestor, before it emits replacement
`BlockApplied` values. Forks deeper than the configured window are terminal.
...
flowchart LR
    ...
    block -->|shallow reorg correction| reverted[ChainEvent BlockReverted]

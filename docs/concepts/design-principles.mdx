...
```mermaid
flowchart LR
    rpc[EVM JSON-RPC] --> alloy[Alloy source]
    reth[Reth ExEx source] -. planned .-> event[Normalized ChainEvent]
    alloy --> event
...
Only the Alloy path and generic plugin dispatch exist today. The Reth adapter
and example domain plugins in the diagram are planned.
...
| --- | --- | --- |
| Source | Convert source-native data into `ChainEvent` | Implemented by `raven-source-alloy` |
...
| CLI | Compose a source, runtime, and configured plugins | Alloy plus a built-in block logger today |
...
Use a **source adapter** when the input mechanism changes, for example Alloy
polling versus Reth ExEx notifications. Use a **plugin** when application

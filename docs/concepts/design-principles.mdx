...
```mermaid
flowchart LR
    rpc[EVM JSON-RPC] --> rpcSource[RPC source backed by Alloy]
...
| --- | --- | --- |
| Source | Convert source-native data into `ChainEvent` | Implemented for JSON-RPC by `raven-source-rpc` |
...
every sibling mailbox is still attempted. The standalone CLI adds a separate
at-least-once checkpoint gate: it advances durable progress only after every
plugin succeeds and replays an unacknowledged event after restart. Embedders
using `raven-runtime` directly must own an equivalent policy if they require
durability.

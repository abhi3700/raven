---
title: "RPC source"
description: "How Raven's RPC source combines HTTP polling or WebSocket subscriptions with ordered reconciliation."
---

`RpcSource` is the adapter behind `raven run`. It connects to
...
The crate uses Alloy for its provider and native RPC types. The CLI does not
expose `--source alloy` because the implementation library is not an
operator-selected deployment mode. The endpoint may be hosted or self-managed;
the URL scheme selects the transport behavior.
...
use raven_source_rpc::{RpcSource, SourceStart};
use tokio::sync::mpsc;
...
let source = RpcSource::new("wss://your-evm-rpc.example")
...
```rust
use raven_source_rpc::{RpcSource, BlockFetchMode};

let source = RpcSource::new("https://your-evm-rpc.example")
...
`RpcSourceError::is_transient` separates retryable transport/state failures

...
`AlloySource` is Raven's current chain adapter. It connects to EVM-compatible
...
flowchart LR
    ...
    event --> first[First event initializes Runtime]
    first --> context[PluginContext carries detected ID]
```
